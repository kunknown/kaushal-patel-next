...
import { getExampleTable } from "./database";
...
export async function submitLoginData(formData: FormData) {
  try {
    ...
    getExampleTable();
    // const kpSchema = pgSchema("kp_schema");
    // const kpSchemaUsers = kpSchema.table("users", {
    //   createdAt: timestamp("createdAt").defaultNow().notNull(),
    //   email: text("email").unique().notNull(),
    //   id: serial("id").primaryKey().unique(),
    //   name: text("name").notNull(),
    ...
    //   })
    //   .returning();
    // const result = await db.select().from(kpSchemaUsers);
...
// export async function authenticate({ email, password }: TAuthData) {
//   try {
//     await mongooseConnect(mongoose);
